...
# to install openssl (which requires installing perl....).
...
# 7.xx download script - we'll stick it in /root/.local/bin so we can
# also copy it into the BLFS stage
COPY scripts/download_file.py /root/.local/bin/download_file.py
COPY scripts/7.xx-download.sh /lfs-scripts/7.xx-download.sh
RUN /lfs-scripts/7.xx-download.sh
